...
from .selenium_helper import getXMLheader
...
def get_xml_departure(hash='', id=''):
    xml = """<?xml version='1.0' encoding='utf-8'?><soap:Envelope xmlns:xsi='http://www.w3.org/2001/XMLSchema-instance' xmlns:xsd='http://www.w3.org/2001/XMLSchema' xmlns:soap='http://schemas.xmlsoap.org/soap/envelope/'><soap:Body><CNR_GetRealDepartures xmlns='http://PublicService/'><id>""" + '{}'.format(
        id) + """</id><s>""" + hash + """</s></CNR_GetRealDepartures></soap:Body></soap:Envelope>"""
    return xml
...
class ZTM:
    ...
    def getBusStop(self, id='757'):
        ...
        body = xml_parsed['soap:Envelope']['soap:Body']['CNR_GetRealDeparturesResponse']['CNR_GetRealDeparturesResult'][
            'Schedules']
        stop = body['Stop']
        day = stop['Day']
        R = day['R']
    ...
    def requestDeparture(self, id='757'):
        ...
        cache_name = f"bus_stop_{id}"
        val = str(new_json)
        cache.set(cache_name, val)
